...
func TestIsGumCancel(t *testing.T) {
	t.Run("returns true for exit code 1", func(t *testing.T) {
		exitErr := makeExitErrorWithCode(t, 1)
	...
	t.Run("returns true for exit code 130", func(t *testing.T) {
		exitErr := makeExitErrorWithCode(t, 130)
		if !isGumCancel(exitErr) {
	...
	t.Run("returns false for exit code 2", func(t *testing.T) {
		exitErr := makeExitErrorWithCode(t, 2)
		if isGumCancel(exitErr) {
	...
	t.Run("returns false for non-ExitError", func(t *testing.T) {
		if isGumCancel(fmt.Errorf("some error")) {
...
func TestRefinePluginSelection_GumCrash(t *testing.T) {
	t.Run("returns error on gum crash", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "plugin selection failed") {
			t.Errorf("expected wrapping error, got %q", err.Error())
		}
	...
	t.Run("silent on user cancellation and returns pre-selected", func(t *testing.T) {
		exitErr := makeExitErrorWithCode(t, 1)
		wio, errBuf := testGumWizardIO(func(args ...string) ([]byte, error) {
			return nil, exitErr
		})
