...
// warnIfGumCrash writes a warning to w if err indicates an unexpected
// gum failure (not a deliberate user cancellation).
func warnIfGumCrash(err error, w io.Writer, action string) {
	if err == nil || isGumCancel(err) {
		return
	}
...
func SelectMarketplaces(wio WizardIO, available []Marketplace) ([]Marketplace, error) {
	...
	if err != nil {
		...
		warnIfGumCrash(err, wio.Err, "marketplace selection failed")
...
func selectCategories(wio WizardIO, categories []Category) ([]Category, error) {
	...
	if err != nil {
		...
		warnIfGumCrash(err, wio.Err, "category selection failed")
...
func refinePluginSelection(wio WizardIO, availablePlugins []string, installed map[string]bool) ([]string, error) {
	...
	if err != nil {
		...
		warnIfGumCrash(err, wio.Err, "plugin selection failed")
...
func PromptForDescription(wio WizardIO, autoGenerated string) (string, error) {
	...
	if err := wio.gumExec("confirm", confirmMsg); err != nil {
		...
		warnIfGumCrash(err, wio.Err, "confirmation prompt failed")
...
func editDescription(wio WizardIO, placeholder string) (string, error) {
	...
	if err != nil {
		...
		warnIfGumCrash(err, wio.Err, "editor failed")
