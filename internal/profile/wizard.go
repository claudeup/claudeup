...
func PromptForDescription(autoGenerated string) (string, error) {
	// Show auto-generated description
	fmt.Printf("  %s\n\n", autoGenerated)
	...
	// Ask if user wants to accept or edit
	cmd := exec.Command("gum", "confirm", "Accept this description?")
	cmd.Stdin = os.Stdin
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		// User said no or cancelled - open editor
...
func fallbackDescriptionPrompt(autoGenerated string) (string, error) {
	fmt.Print("Accept this description? [Y/n]: ")
