...
// ErrGumCancelled indicates the user cancelled a gum prompt (Esc, Ctrl+C, or decline).
// Callers can use errors.Is(err, ErrGumCancelled) for structural cancel detection
// instead of string-matching on error messages.
var ErrGumCancelled = errors.New("gum: user cancelled")
...
func SelectMarketplaces(wio WizardIO, available []Marketplace) ([]Marketplace, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			return nil, fmt.Errorf("marketplace selection cancelled: %w", ErrGumCancelled)
		}
...
func selectCategories(wio WizardIO, categories []Category) ([]Category, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			return nil, fmt.Errorf("category selection cancelled: %w", ErrGumCancelled)
		}
...
// On user cancellation, it returns the auto-generated description with an
// error wrapping ErrGumCancelled. Callers should use the returned value
// even when err wraps ErrGumCancelled.
func PromptForDescription(wio WizardIO, autoGenerated string) (string, error) {
	...
	if err := wio.gumExec("confirm", confirmMsg); err != nil {
		if isGumCancel(err) {
			return autoGenerated, fmt.Errorf("description editing declined: %w", ErrGumCancelled)
		}
...
// On user cancellation, it returns the placeholder with an error wrapping
// ErrGumCancelled. Callers should use the returned value even when err
// wraps ErrGumCancelled.
func editDescription(wio WizardIO, placeholder string) (string, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			return placeholder, fmt.Errorf("description editing cancelled: %w", ErrGumCancelled)
		}
