...
// error wrapping ErrGumCanceled. Callers should use the returned value
// even when err wraps ErrGumCanceled.
func PromptForDescription(wio WizardIO, autoGenerated string) (string, error) {
	...
	if err := wio.gumExec("confirm", confirmMsg); err != nil {
		if isGumCancel(err) {
			return autoGenerated, fmt.Errorf("description editing declined: %w", ErrGumCanceled)
		}
...
// ErrGumCanceled. Callers should use the returned value even when err
// wraps ErrGumCanceled.
func editDescription(wio WizardIO, placeholder string) (string, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			return placeholder, fmt.Errorf("description editing cancelled: %w", ErrGumCanceled)
		}
