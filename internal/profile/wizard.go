...
// isGumCancel reports whether err looks like a deliberate user cancellation.
//
// Gum signals user cancellation via specific exit codes:
//   - 1:   user declined, aborted, or pressed Esc
//   - 130: user pressed Ctrl+C (SIGINT)
//
// All other errors (including nil) return false.
// A gum internal crash that happens to exit 1 or 130 would be
// misclassified as a cancellation; this is accepted given gum's
// exit code conventions.
func isGumCancel(err error) bool {
...
// announcePluginFallback tells the user that a cancelled or skipped plugin
// refinement kept the pre-selected plugins rather than aborting the wizard.
func announcePluginFallback(wio WizardIO, preselectedCount int) {
	if preselectedCount == 0 {
		fmt.Fprintln(wio.Out, "No plugins selected")
		return
	}
	fmt.Fprintf(wio.Out, "Using pre-selected plugins (%d)\n", preselectedCount)
}

// announceDescriptionFallback tells the user that a declined or cancelled
// description edit kept the auto-generated description.
func announceDescriptionFallback(wio WizardIO) {
	fmt.Fprintln(wio.Out, "Using auto-generated description")
}
...
func SelectMarketplaces(wio WizardIO, available []Marketplace) ([]Marketplace, error) {
	...
	output, err := wio.gumOutput(args...)
	if err != nil {
		if isGumCancel(err) {
			return nil, fmt.Errorf("marketplace selection cancelled: %w", ErrGumCanceled)
		}
		return nil, fmt.Errorf("marketplace selection failed: %w", err)
	}
...
func selectCategories(wio WizardIO, categories []Category) ([]Category, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			return nil, fmt.Errorf("category selection cancelled: %w", ErrGumCanceled)
		}
...
// Installed plugins are pre-selected by default. Cancelling keeps the pre-selected
// plugins and announces that on wio.Out so the fallback is visible.
func refinePluginSelection(wio WizardIO, availablePlugins []string, installed map[string]bool) ([]string, error) {
	...
	if err != nil {
		if isGumCancel(err) {
			...
			announcePluginFallback(wio, len(preselected))
...
func fallbackPluginRefinement(wio WizardIO, availablePlugins []string, installed map[string]bool) ([]string, error) {
	...
	if input == "" {
		...
		announcePluginFallback(wio, len(result))
		return result, nil
	}
...
// On user cancellation, it returns the auto-generated description with a nil
// error -- declining to edit is an affirmative choice to keep the default --
// and announces that on wio.Out so the fallback is visible.
func PromptForDescription(wio WizardIO, autoGenerated string) (string, error) {
	...
	if err := wio.gumExec("confirm", confirmMsg); err != nil {
		if isGumCancel(err) {
			announceDescriptionFallback(wio)
...
// On user cancellation, it returns the placeholder with a nil error --
// declining to edit is an affirmative choice to keep the default --
// and announces that on wio.Out so the fallback is visible.
func editDescription(wio WizardIO, placeholder string) (string, error) {
	output, err := wio.gumOutput("write", "--placeholder", placeholder, "--header=Edit description (Ctrl+D to save):")
	if err != nil {
		if isGumCancel(err) {
			announceDescriptionFallback(wio)
